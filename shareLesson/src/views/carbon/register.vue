<template>
  <div class="content">
    <div class="favourable">
      ...
      <p class="f2"><input type="text" placeholder="请输入验证码" v-model="code" style="width: 100px;" maxlength="6"> <button @click="sendregistercode()" id="btn1">发送</button></p>
      <p class="f2"><input type="password" placeholder="请设置6-16位密码" v-model="password"></p>
      <p class="f3"><input type="password" placeholder="请确认密码" v-model="passwordConfig"></p>
  ...
  methods: {
    ...
    logins(){
     ...
     goregister(){
      ...
      if(this.password != ""){
        if ( this.password != this.passwordConfig ){
          alert('两次输入的密码不一致')
          return false
        }
